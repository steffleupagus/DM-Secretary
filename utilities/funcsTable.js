...
//const { MessageMentions, TextInputStyle } = require('discord.js')
...
const RPUtils = require(`../utilities/roleplayUtils.js`)
...
{
	if (!thread) throw "Table cannot be awarded without the RP thread."
	if (!table) table = await ChanUtils.isTableRPThread(thread)
	if (!table) throw "Table record could not be found."

	console.log("Award Table: " + thread.name + "\n", table)
	var rpData = await MsgUtils.getAllRoleplayData(thread);
	const start = rpData.start
	delete rpData.tupperMap
	delete rpData.start
	rpData = await RPUtils.processRoleplayData(rpData, interaction, thread)

	//Sort the DM's data 
	keys = { "user":SortOrder.ASC, "char":SortOrder.ASC, "level":SortOrder.DESC };
	rpData.sort((a,b)=>{ return Utils.priorityCompare(a, b, keys) })	
	console.log("\nRP Data:\n",util.inspect(rpData, false, null, true /* enable colors */))
	
	// await sendDMApprovalMessage(message, start, rpData, "Scene auto-closed");	

	// const embeds = generatePlayerConfirmEmbed(rpData)
...
// const dmPingChannel = Debug ? config.debugChannels.dmPing : config.dmPingChannel;
// const xpLogChannel  = Debug ? config.debugChannels.dmPing : config.xpLogChannel;
...
// 	try	{ 
// 		await sendDMApprovalMessage(interaction, start, rpData, "");
// 	}
// 	catch(err)  { Mutex.unlock(channel, err) }

// 	// await LogDebugResult(interaction, rpData, start, interaction.channel, "");
// 	// await LogDebugResult(interaction, pcData, start, interaction.channel, "");
